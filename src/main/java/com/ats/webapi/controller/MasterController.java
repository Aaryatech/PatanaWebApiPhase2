	 ...
	 //------------------------------------------------------------------------				
	          ...
	          //------------------------------------------------------------------------
				...
				public List<GetRegSpCakeOrders> getRegSpCakeOrderHistory(
						@RequestParam String spDeliveryDt,@RequestParam int  frId,@RequestParam int catId, int searchBy) {
					List<GetRegSpCakeOrders> regSpCakeOrder = regularSpCkOrderService.getRegSpCakeOrderHistory(spDeliveryDt, frId,catId,searchBy);
					...
					try {
						for (String frId : frIdList) {
							...
							if (configureFr != null) {
								...
								//System.err.println("commas seperated  itemShow =" + intArray.stream().map(String::valueOf).collect(Collectors.joining(",")));
