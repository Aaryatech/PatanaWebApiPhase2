...
public class RestApiController {
	...
	public @ResponseBody GetGrnGvnForCreditNoteList grnGvnDetailForCreditNote(@RequestParam("isGrn") int isGrn,
		...
		// System.out.println("Rest : is Grn Received /grnGvnDetailForCreditNote " +
		// isGrn);
	...
	public @ResponseBody Info updateStoreGvn(@RequestBody List<TempGrnGvnBeanUp> dataList) {
		...
		// System.out.println("inside rest /updateStoreGvn : input para = dataList " +
		// dataList.toString());
	...
	public @ResponseBody Info updateGateGrn(@RequestBody List<TempGrnGvnBeanUp> dataList) {
		...
		Info info = new Info();
		try {
			// System.out.println("inside rest /updateGateGrn : input para = dataList " +
			// dataList.toString());
			...
			// System.out.println("/Rest Api Exce in Updating Gate Grn Gvn Record
			// /updateGateGrn" + e.getMessage());
			e.printStackTrace();
		}
	...
	public @ResponseBody Info updateAccGrn(@RequestBody List<TempGrnGvnBeanUp> dataList) {
		...
		Info info = new Info();
		try {
			// System.out.println("inside rest /updateAccGrn : Param " +
			// dataList.toString());
	...
	public @ResponseBody GetBillsForFrList getBillsForFrByBillDate(@RequestParam("frId") int frId,
		...
		// System.out.println("GEt BillS for Fr by Bill Date " +
		// billsForFrLisr.toString());
	...
	// 21-10-2020 akshay
	@RequestMapping(value = "/getBillsForFrBetweenBillDate", method = RequestMethod.POST)
	public @ResponseBody GetBillsForFrList getBillsForFrBetweenBillDate(@RequestParam("frId") int frId,
			@RequestParam("fromDate") String fromDate, @RequestParam("toDate") String toDate) {

		// System.out.println("Fr Id ::: " + frId);

		GetBillsForFrList billsForFrLisr = getBillsForFrService.getBillsForFrBetweenBillDate(frId, fromDate, toDate);
	...
	public @ResponseBody Info updateBillDetailforGrnGvnInsertint(@RequestParam("billDetailNo") int billDetailNo) {
		...
		try {
			...
			// System.out.println("Exce in update bill Detail For GRN GVN Rest Api
			// /updateBillDetailforGrnGvnInsert " + e.getMessage());
			e.printStackTrace();
		}
	...
	public @ResponseBody GetGrnItemConfigList getGrnItemConfig(@RequestParam("frId") int frId) {
		...
		try {
			...
			// System.out.println("grn Item config with Rest: " +
			// grnItemConfigList.toString());
	...
	public @ResponseBody GetGrnItemConfigList getItemsForManGrn(@RequestParam("frId") int frId,
		...
		try {
			...
			// System.out.println("grn Item getItemForManualGrn Rest: " +
			// grnItemConfigList.toString());

		} catch (Exception e) {

			// System.out.println("restApi Exce for Getting Man GRN Item Conf
			// /getItemsForManGrn" + e.getMessage());
			e.printStackTrace();
		}
	...
	public @ResponseBody String updateBillDetails(@RequestParam List<PostBillDetail> postBillDetailsList) {
		...
		// System.out.println("post bill Detail with get bill Details= " +
		// postBillDetailsList.toString());
	...
	public @ResponseBody FrNameIdByRouteIdList getFrNameIdByRouteId(@RequestParam("routeId") List<Integer> routeId) {
...
//	}
	...
	@RequestMapping(value = { "/updateBillNoInForSpCake" }, method = RequestMethod.POST)
	public @ResponseBody Info updateBillNoInForSpCake(@RequestParam("spOrderNo") int spOrderNo,
		...
		try {
			int update = spCakeOrdersRepository.updateBillNoInForSpCake(spOrderNo, billNo);

			if (update > 0) {
				info.setError(false);
				info.setMessage("generated");
			} else {
				info.setError(true);
				info.setMessage("some error while genrating Bill No.");
			}
		} catch (Exception e) {
			e.printStackTrace();
			info.setError(true);
	...
	public @ResponseBody SpCkOrderHisList searchSpCakeOrderHistory(@RequestParam String spDeliveryDt, String frCode,
		...
		SpCkOrderHisList spCakeOrderList = spCakeOrdersService.searchOrderHistory(spDeliveryDt, frCode, searchBy);
	...
	public SpHistoryExBill getSpCkOrderForExBill(@RequestParam("orderNo") String orderNo,
			...
			@RequestParam("frId") int frId, @RequestParam("searchBy") int searchBy) {
		...
		try {
			if (orderNo.equalsIgnoreCase("0")) {
				...
				if (searchBy == 2) {
					List<SpCkOrderHis> spCakeOrder = spCakeOrderHisRepository.findByOrderDateForExBill(date, menuId,
							frId);
					List<GetRegSpCakeOrders> regularSpCkOrders = getRegSpCakeOrdersRepository
							.findByOrderDateForExBill(date, menuId, frId);
					...
					List<GetRegSpCakeOrders> regularSpCkOrders = getRegSpCakeOrdersRepository
							.findByOrdersForExBill(date, menuId, frId);
					spHistoryExBill.setSpCakeOrder(spCakeOrder);
					spHistoryExBill.setRegularSpCkOrders(regularSpCkOrders);
				}
	...
	public @ResponseBody ItemOrderList searchOrderHistory(@RequestParam int searchBy, @RequestParam int catId,
		...
		ItemOrderList orderList = orderService.searchOrderHistory(catId, deliveryDate, frId, searchBy);
	...
	public @ResponseBody Info updateFrConf(@RequestParam("frIdList") List<Integer> frIdList,
		...
		try {
			if (frIdList.contains(0)) {
				...
				result = connfigureService.updateFrConfForSelectedFr(frIdList, menuId, fromTime, toTime, date, day,
						settingType);
			}
...
//			specialcake.setIsSlotUsed(isSlotUsed);
	...
	// Get All Items By Sub Category Id
		@RequestMapping(value = { "/getAllItemsBySubCatId" }, method = RequestMethod.POST)
		public @ResponseBody List<GetItemUomAndSup> getAllItemsBySubCatId(@RequestParam String subCatId) {
			List<GetItemUomAndSup> itemsList = itemService.findAllItemsBySubCatId(subCatId);
			return itemsList;
		}
...
//b
	...
	public RegSpCkOrderResponse getRegSpCkOrderList(@RequestParam List<Integer> frId, @RequestParam String prodDate,
		...
		try {
			...
			// System.out.println("exception in sp cake order list for all fr rest
...
@Autowired GetDumpOrderRepository dumpRepo;
	...
	public @ResponseBody List<GetSellBillHeader> getSellBillHeader(@RequestParam("fromDate") String fromDate,
		...
		// System.out.println("List Sell Bill Header**** " +
	...
	public @ResponseBody List<GetSellBillDetail> getSellBillDetail(@RequestParam("sellBillNo") int sellBillNo) {
		...
		// System.out.println("List Sell Bill Detail**** " +
		// getSellBillDetailList.toString());
	...
	public @ResponseBody Info info(@RequestBody List<FrItemStockConfigurePost> frItemStockConfigurePosts)
		...
		try {
			...
			// System.out.println("Exce in frItem Stock Insertion : RestApi" +
			// e.getMessage());
			e.printStackTrace();
		}
...
//	}
	...
	@Autowired
	TestFrRepository testFrRepository;
