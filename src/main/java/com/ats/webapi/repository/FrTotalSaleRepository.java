...
public interface FrTotalSaleRepository extends JpaRepository<FrTotalSale, Integer>{
	...
	@Query(value="select fr_id, EXTRACT(MONTH FROM :firstDate) as month,ifnull(SUM(grand_total),0) as total_sale from t_sell_bill_header where fr_id=:frId AND bill_date  Between :firstDate AND :lastDate",nativeQuery=true)
