...
public interface GenerateBillRepository extends JpaRepository<GenerateBill, Integer> {
	...
	/*
	 * @Query(
	 * value=" SELECT t_order.order_id ,t_order.is_edit as sp_delivery_place,t_order.is_positive,t_order.fr_id, t_order.menu_id,t_order.grn_type ,t_order.item_id,t_order.edit_qty as order_qty,"
	 * +
	 * " t_order.order_rate,t_order.order_mrp ,t_order.delivery_date, CONCAT(m_franchisee.fr_name,' ',m_franchisee.fr_code) AS fr_name,m_franchisee.is_same_state,m_franchisee.fr_code,m_franchisee.fr_rate_cat, m_fr_menu_show.menu_title,"
	 * +
	 ...
	 * + "",nativeQuery=true) List<GenerateBill>
	 * generateBill(@Param("frId")List<String> frId,@Param("menuId")List<String>
	 ...
	 * " t_order.menu_id=m_fr_menu_show.menu_id AND t_order.item_id=m_item.id AND t_order.is_bill_generated IN(0,1) AND t_order.edit_qty!=0 ORDER BY m_franchisee.fr_id,m_item.item_grp1,m_item.item_grp2,m_item.item_name "
	 * + "",nativeQuery=true) List<GenerateBill>
	 * generateBillForAllFr(@Param("menuId")List<String>
	 * menuId,@Param("delDate")String delDate);
	 * 
	 * 
	 * 
	 ...
	 * " t_order.menu_id=m_fr_menu_show.menu_id AND t_order.item_id=m_item.id AND t_order.is_bill_generated IN(0,1) AND t_order.edit_qty!=0 ORDER BY m_franchisee.fr_id,m_item.item_grp1,m_item.item_grp2,m_item.item_name "
	 * + "",nativeQuery=true) List<GenerateBill>
	 * generateBillForAllFrAllMenu(@Param("delDate")String delDate);
	 * 
	 * 
	 * 
	 * 
	 ...
	 * value=" SELECT t_order.order_id , t_order.is_edit as sp_delivery_place,t_order.is_positive,t_order.fr_id, t_order.menu_id,t_order.grn_type,t_order.item_id,t_order.edit_qty as order_qty,"
	 * +
	 * " t_order.order_rate,t_order.order_mrp ,t_order.delivery_date, CONCAT(m_franchisee.fr_name,' ',m_franchisee.fr_code) AS fr_name,m_franchisee.fr_code,m_franchisee.is_same_state,m_franchisee.fr_rate_cat, m_fr_menu_show.menu_title,"
	 * +
	 * " m_item.item_name,m_item.item_grp1,m_item.item_grp2,m_item.item_tax1, m_item.item_tax2, m_item.item_tax3,m_item.item_shelf_life from t_order,m_franchisee, m_fr_menu_show ,"
	 * +
	 * " m_item where t_order.fr_id IN (:frId) AND t_order.delivery_date=:delDate "
	 ...
	 * " t_order.menu_id=m_fr_menu_show.menu_id AND t_order.item_id=m_item.id AND t_order.is_bill_generated IN(0,1) AND t_order.edit_qty!=0 ORDER BY m_franchisee.fr_id,m_item.item_grp1,m_item.item_grp2,m_item.item_name "
	 * + "",nativeQuery=true) List<GenerateBill>
	 * generateBillForAllMenu(@Param("frId")List<String>
	 * frId,@Param("delDate")String delDate);
	 * 
	 * 
	 * @Query(
	 * value=" SELECT t_order.order_id ,t_order.is_positive,t_order.is_edit as sp_delivery_place,t_order.fr_id, t_order.menu_id,t_order.grn_type,t_order.item_id,t_order.edit_qty as order_qty,"
	 * +
	 ...
	 * + "",nativeQuery=true) List<GenerateBill>
	 * getBillOfOrder(@Param("idList")ArrayList<Integer> list);
	 */
