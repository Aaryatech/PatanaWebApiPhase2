...
public interface GetGrnItemConfigRepository extends JpaRepository<GetGrnItemConfig, Integer> {
	...
	@Query(value = "SELECT t_bill_header.bill_no,t_bill_header.bill_date_time," + 
			...
			+ "t_bill_detail.expiry_date=:cDate AND t_bill_detail.is_grngvn_applied=0 and t_bill_detail.cat_id NOT IN(4,3)", nativeQuery = true)
	
	public List<GetGrnItemConfig> getAllGrnItemConfig1(@Param("cDate") String cDate,@Param("frId") int frId);
	...
	@Query(value = "SELECT t_bill_header.bill_no,t_bill_header.bill_date_time," + 
			...
			"t_bill_header.fr_id,t_bill_header.invoice_no FROM t_bill_header ,t_bill_detail,m_item " + 
			" WHERE t_bill_header.fr_id=:frId AND t_bill_detail.bill_no=t_bill_header.bill_no AND t_bill_header.status=2 " + 
			"AND t_bill_detail.item_id=m_item.id AND  t_bill_detail.grn_type !=0"
	...
	*/
	...
	@Query(value = "SELECT data.*,\n" + 
			"CASE  WHEN data.cat_id=5 THEN b.sp_name   \n" + 
			"            ELSE  a.item_name \n" + 
			"        END AS item_name \n" + 
			"from (\n" + 
			"        select t_bill_header.bill_no,\n" + 
			...
			"    group by\n" + 
			"        t_bill_detail.bill_detail_no) data\n" + 
			"    left join (\n" + 
			"    select m_item.item_name, m_item.id from m_item \n" + 
			"    \n" + 
			"    ) a on data.item_id=a.id\n" + 
			"    left join (\n" + 
			"    select s.sp_name,s.sp_id  from m_sp_cake s \n" + 
			"    \n" + 
			"    ) b on data.item_id=b.sp_id", nativeQuery = true)
