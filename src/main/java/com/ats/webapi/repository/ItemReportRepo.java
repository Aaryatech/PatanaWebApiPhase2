...
public interface ItemReportRepo extends JpaRepository<ItemReport, Integer> {
	...
	@Query(value = "SELECT d.item_id,SUM(d.bill_qty) as bill_qty,SUM(d.order_qty) as order_qty,i.item_name,h.bill_no,h.invoice_no, d.cat_id   "
	...
	@Query(value = "SELECT d.item_id, d.cat_id,SUM(d.bill_qty) as bill_qty,SUM(d.order_qty) as order_qty,i.item_name,h.bill_no,h.invoice_no  "
			+ " FROM t_bill_header h,t_bill_detail d,m_item i WHERE h.del_status=0 AND d.del_status=0 AND i.id=d.item_id AND d.cat_id!=5 AND "
