...
public interface RegularSpCkItemsRepository extends JpaRepository<GetRegularSpCkItems, Integer> {
	 ...
	 @Query(value = "SELECT id,item_name,item_image,item_rate1,item_rate2,item_rate3,item_mrp1,item_mrp2,item_mrp3,item_shelf_life,Item_tax1,Item_tax2,Item_tax3 FROM `m_item` WHERE item_grp2 =:itemGrp2 and id IN(:items) order by item_name",nativeQuery=true)
