...
public interface RegularSpCkOrderAdminRepo extends JpaRepository<RegularSpCkOrders, Long> {
	...
	List<RegularSpCkOrders> findRegularSpCakeOrderAllMenu(@Param("frId")List<Integer> frId, @Param("strDate")String strDate);
	
	
	@Query(value="SELECT t_regular_sp_cake.rsp_id, CONCAT(m_franchisee.fr_name,' ',m_franchisee.fr_code) AS fr_name ,m_item.item_name ,m_item.id,t_regular_sp_cake.rsp_delivery_dt, t_regular_sp_cake.rsp_events,t_regular_sp_cake.rsp_events_name,t_regular_sp_cake.rsp_sub_total,t_regular_sp_cake.tax_1,t_regular_sp_cake.tax_1_amt,t_regular_sp_cake.tax_2,t_regular_sp_cake.tax_2_amt,t_regular_sp_cake.rate,t_regular_sp_cake.mrp,t_regular_sp_cake.qty FROM m_franchisee ,m_item,t_regular_sp_cake WHERE t_regular_sp_cake.rsp_produ_date =:strDate and t_regular_sp_cake.del_status=0 AND t_regular_sp_cake.fr_id IN (:frId) AND t_regular_sp_cake.item_id = m_item.id AND t_regular_sp_cake.fr_id = m_franchisee.fr_id and t_regular_sp_cake.menu_id IN (:menuId) ",nativeQuery=true)
	List<RegularSpCkOrders> findRegularSpCakeOrderOneMenu(@Param("frId")List<Integer> frId, @Param("strDate")String strDate,@Param("menuId") List<Integer> menuId);
