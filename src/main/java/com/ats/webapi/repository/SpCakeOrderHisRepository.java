...
public interface SpCakeOrderHisRepository extends JpaRepository<SpCkOrderHis, Long> {
	...
	@Query(value="select t_sp_cake.*,m_sp_flavour.spf_name,m_sp_cake.sp_name from t_sp_cake,m_sp_flavour,m_sp_cake where t_sp_cake.order_date=:spDeliveryDate AND  t_sp_cake.fr_code=:frCode AND t_sp_cake.sp_flavour_id=m_sp_flavour.spf_id AND t_sp_cake.sp_id=m_sp_cake.sp_id",nativeQuery=true)
	List<SpCkOrderHis> findByMenuIdInAndSpOrderDt(@Param("spDeliveryDate")String spDeliveryDate,@Param("frCode")String frCode);
	
	@Query(value="select t_sp_cake.*,m_sp_flavour.spf_name,m_sp_cake.sp_name from t_sp_cake,m_sp_flavour,m_sp_cake where t_sp_cake.order_date=:date and t_sp_cake.fr_id=:frId And  t_sp_cake.menu_id In(:menuId) and t_sp_cake.sp_flavour_id=m_sp_flavour.spf_id AND t_sp_cake.sp_id=m_sp_cake.sp_id",nativeQuery=true)
	List<SpCkOrderHis> findByOrderDateForExBill(@Param("date")String date,@Param("menuId")List<String> menuId,@Param("frId") int frId);
}
